...
from gluoncv.torch.data import multiGridHelper, MultiGridBatchSampler
...
class VideoClsDataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.mode == 'train':
            ...
            sample = self.dataset_samples[index]
            # print(sample)
            clip_ind = self.clip_inds[index]  # -1
            # sys.exit()
            buffer, all_index = self.loadvideo_decord(sample, clip_ind) # len 16
            # print(len(buffer))
            if len(buffer) == 0:
                while len(buffer) == 0:
            ...
            # return buffer, self.label_array[index], sample.split("/")[-1].split(".")[0]
            return buffer, self.label_array[index], sample.split(".")[0], clip_ind, all_index
